...
import { ChevronLeft, ChevronRight, Plus, Clock, Loader2, Users } from "lucide-react";
...
export default function WorkspaceCalendarPage() {
  ...
  const handleDeleteEvent = async (eventId: string) => {
    try {
      await api.delete(`/organizations/${orgId}/events/${eventId}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-5 border border-border rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground flex items-center gap-1"><Users className="w-3 h-3" /> Meeting & acara seluruh workspace</p>
          </div>
        </div>
        {canManage && (
          <button onClick={() => setIsModalOpen(true)}
            ...
            <Plus className="w-4 h-4" /> Meeting Baru
          </button>
        )}
      </div>
      ...
      <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-xl">
        ...
        {loading ? (
          <div className="h-[600px] flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-muted-foreground" /></div>
        ) : (
          <div className="grid grid-cols-7 auto-rows-[120px] md:auto-rows-[160px]">
            {totalDays.map((day, i) => {
              ...
              return (
                <div key={i} className={cn("border-r border-b border-border p-3 space-y-2 hover:bg-secondary/10 transition-colors group relative",
                  (i + 1) % 7 === 0 && "border-r-0", !day && "bg-secondary/5")}>
                  {day && (
                    <>
                      <div className={cn("w-8 h-8 flex items-center justify-center text-xs font-bold rounded-full transition-all",
                      ...
                      <div className="space-y-1.5 overflow-y-auto max-h-[80px] md:max-h-[110px] custom-scrollbar">
                        {dayEvents.map((event) => (
                          <div key={event.id} onClick={() => { setSelectedEvent(event); setIsDetailOpen(true); }}
                            className="px-2 py-1.5 bg-primary/5 border-l-2 border-primary rounded-r-md text-[9px] font-bold text-primary hover:bg-primary/10 transition-all cursor-pointer truncate" title={event.title}>
                            <div className="flex items-center gap-1"><Clock className="w-2.5 h-2.5 shrink-0" />{new Date(event.start_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                            {event.title}
                          </div>
                        ))}
                      </div>
      ...
      <AddEventModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} orgId={orgId as string} onSuccess={fetchData} />
      <EventDetailModal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} event={selectedEvent} onDelete={handleDeleteEvent} onUpdate={fetchData} canManage={canManage} />
    </div>
  );
}
