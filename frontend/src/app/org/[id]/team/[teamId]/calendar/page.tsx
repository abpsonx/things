...
export default function TeamCalendarPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [teamRes, eventsRes, tasksRes] = await Promise.all([
                      ...
                      {day && (
                        <>
                          ...
                          <div className="space-y-1 overflow-y-auto max-h-[80px] md:max-h-[100px]">
                            {dayEvents.map((e) => (
                              <div key={e.id} className="group/ev px-2 py-1 bg-primary/10 border-l-2 border-primary rounded-r-md text-[9px] font-bold text-primary truncate flex items-center justify-between gap-1" title={e.title}>
          ...
          <div className="bg-card rounded-3xl w-full max-w-md shadow-2xl border border-border overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              ...
              <textarea
                ...
                className="w-full px-3 py-2.5 rounded-2xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm min-h-[80px] resize-none"
              />
            </div>
