...
export default function EventDetailModal({ isOpen, onClose, event, onDelete, onUpdate }: EventDetailModalProps) {
  ...
  const handleRSVP = async (status: "accepted" | "declined") => {
    try {
      ...
      await api.patch(`/projects/${projectId}/events/${event.id}/rsvp`, { status });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Detail Event">
      <div className="space-y-6 p-1">
        ...
        {event.description && (
          <div className="space-y-2">
            ...
            <div className="p-4 bg-secondary/10 border border-border rounded-2xl text-sm leading-relaxed whitespace-pre-wrap">
              {event.description}
            </div>
          </div>
        )}
        ...
        {event.attendees && event.attendees.length > 0 && (
          <div className="space-y-3">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1">
              <Users className="w-3 h-3" /> Anggota yang Diundang
            </span>
            <div className="flex flex-wrap gap-2">
              {event.attendees.map((att: any) => (
                <div key={att.id} className={cn(
                  "flex items-center gap-2 border rounded-full pl-1 pr-3 py-1",
                  att.status === "accepted" ? "border-emerald-500/30 bg-emerald-500/5 text-emerald-600" :
                  att.status === "declined" ? "border-red-500/30 bg-red-500/5 text-red-600" :
                  ...
                  <div className="w-6 h-6 rounded-full bg-background flex items-center justify-center text-[8px] font-bold border border-border/50 overflow-hidden">
                    {att.user?.avatar_url ? (
                      <img src={att.user.avatar_url} alt="" className="w-full h-full object-cover" />
                    ) : (
        ...
        {/* RSVP Section (Only show if current user is an attendee) */}
        {myAttendee && (
          <div className="p-4 border border-border rounded-2xl bg-secondary/10 space-y-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={isSubmitting || myAttendee.status === "accepted"}
                className={cn(
                  ...
                  myAttendee.status === "accepted" 
                    ...
                    : "bg-background border-border hover:bg-emerald-500/10 hover:text-emerald-600 hover:border-emerald-500/30"
                )}
              >
              ...
              <button
                ...
                disabled={isSubmitting || myAttendee.status === "declined"}
                className={cn(
                  "flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold transition-all border",
                  myAttendee.status === "declined" 
                    ? "bg-red-500/20 border-red-500/30 text-red-600" 
                    : "bg-background border-border hover:bg-red-500/10 hover:text-red-600 hover:border-red-500/30"
                )}
              >
        ...
        <div className="pt-4 border-t border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] text-muted-foreground">Pembuat Event</span>
            </div>
          </div>
