    ...
    const initDM = async () => {
      try {
        ...
        socketRef.current.on("connect", () => {
          socketRef.current.emit("join", { room: `dm_${res.data.id}` });
        });

        socketRef.current.on("dm_received", (data: any) => {
          if (data.channel_id === res.data.id) {
            setMessages(prev => {
                ...
                <div className={cn(
                  ...
                  {isMe && (
                    <div className="absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1">
                      <button onClick={() => { setEditingMessage(msg); setNewMessage(msg.content); }} className="p-1 hover:bg-secondary rounded text-muted-foreground"><Edit2 className="w-3 h-3" /></button>
                      <button onClick={() => handleDeleteMessage(msg.id)} className="p-1 hover:bg-destructive/10 rounded text-destructive"><Trash2 className="w-3 h-3" /></button>
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="p-4 border-t border-border bg-card">
        {editingMessage && (
          <div className="mb-2 p-2 bg-secondary/50 rounded-lg flex items-center justify-between text-xs">
            <span className="truncate flex items-center gap-1 text-muted-foreground"><Edit2 className="w-3 h-3" /> Mengedit pesan...</span>
            <button onClick={() => { setEditingMessage(null); setNewMessage(""); }}><X className="w-4 h-4" /></button>
          </div>
        )}
        ...
        {showEmojis && (
          <div className="mb-2 p-2 bg-secondary/30 rounded-xl flex gap-2 overflow-x-auto border border-border shadow-inner">
            {emojis.map(e => (
              <button key={e} onClick={() => { setNewMessage(prev => prev + e); setShowEmojis(false); }} className="text-lg hover:scale-125 transition-transform">{e}</button>
            ))}
          </div>
        )}
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3 bg-secondary/50 rounded-xl hover:bg-secondary transition-colors"><Paperclip className="w-5 h-5 text-muted-foreground" /></button>
          <button type="button" onClick={() => setShowEmojis(!showEmojis)} className="p-3 bg-secondary/50 rounded-xl hover:bg-secondary transition-colors"><Smile className="w-5 h-5 text-muted-foreground" /></button>
